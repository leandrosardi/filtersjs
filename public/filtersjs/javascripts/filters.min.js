filtersJs = {
    ...
    removeValue: function(parent, value) {
        $(parent).find('span[data-myvalue]').filter(function() {
            return $(this).data('myvalue') === value;
        }).remove();
    },
    ...
    addValue: function(parent, value, isPositive) {
        ...
        s = '<span class="label ' + sClass + '" valign="middle" data-myvalue="' + value + '">' +
        "<a href='javascript:void(0)' onclick='filtersJs.removeKeyword(document.getElementById(\""+parent.id+"\"), \""+value+"\");' class='close' data-dismiss='alert'>" +
        " <i class='icon-remove-sign'></i></a> " + value +
        "</span> ";
    ...
    addKeyword: function(parent, isPositive) {
        // grab the raw input value
        var input    = $(parent).find("#keyword").get(0);
        var rawValue = input.value || "";
    
        // split on commas, trim and discard empties
        rawValue
            .split(',')
            .map(function(v){ return v.trim(); })
            .filter(function(v){ return v.length > 0; })
            .forEach(function(value) {
                // add each fragment as its own tag
                filtersJs.addValue(parent, value, isPositive);
        
                // fire the on_add_value callback if present
                var cb = filtersJs.events[parent.id + ".on_add_value"];
                if (typeof cb === 'function') {
                cb(value);
                }
            });
    
        // then clear & reset just like before
        input.value = "";
