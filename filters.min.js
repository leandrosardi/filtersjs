filtersJs = {
    ...
    getPositiveKeywords: function(parent) {
        var arr = [];
        $(".label-info", parent).each(function(index, elem) {
            arr.push($(this).text());
        });
        return alert(arr);
    },
    ...
    getNegativeKeywords: function(parent) {
        var arr = [];
        $(".label-important", parent).each(function(index, elem) {
            arr.push($(this).text());
        });
        return alert(arr);
    },
    ...
    validateValue: function(parent) {
        let jsIndustries = [];
        let value = $('#keyword', parent).val().toLowerCase();

        $('#keywords_list', parent).children().each(function(i, e) {
            jsIndustries.push({
                id: $.trim($(e).val()),
        ...
        if (filtersJs.findValue(parent, value, jsIndustries)) {
    ...
    findValue: function(parent, nameKey, myArray) {
...
console.log("addKeyword.parent.id:"+parent.id);
        input = document.getElementById("keyword", parent)
        transcription = document.getElementById("keywords_transcription", parent)
...
console.log("addKeyword.parent.id:"+parent.id);
        div = document.getElementById("keywords", parent)
...
console.log(parent_id);
        var s =
            ...
            "		<div class='row-fluid'>" +
            "			<div class='span8'>" +
            "				<input onchange='' type='text' class='input-large' name='keyword' id='keyword' list='keywords_list' placeholder='Write "+h['label']+" here' tabindex='2' >" +
        ...
        if (h['allowed_values'] == null) {
            ...
            $.each(h['allowed_values'], function(index, value) {
                //alert(value);  
                $("#keywords_list", parent).append('<option value="' + value + '">' + value + '</option>')
            });
        }

        $('#keyword', parent).on('input', function() {

            if (h['allowed_values'] == null) {
                var i = $.trim($(this).val()).length;
                if (i > 0) {
                    $('#btnKeywordI', parent).prop('disabled', false);
                    $('#btnKeywordE', parent).prop('disabled', false);
                } else {
                    $('#btnKeywordI', parent).prop('disabled', true);
                    $('#btnKeywordE', parent).prop('disabled', true);
                }
